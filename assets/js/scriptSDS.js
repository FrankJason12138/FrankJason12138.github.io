document.getElementById("psychologyTest").onsubmit = function(event) {
    ...
    // 计算标准分
    var standardScore = Math.floor(totalScore * 1.25);
    ...
    function draw(progress) {
        ...
        // 绘制内部滑块
        ctx.fillStyle = 'green';
        if (value >= 53 && value < 62) ctx.fillStyle = 'yellow'; // 轻度抑郁
        if (value >= 62 && value < 72) ctx.fillStyle = 'orange'; // 中度抑郁
        if (value >= 72) ctx.fillStyle = 'red'; // 重度抑郁
        ctx.beginPath();
