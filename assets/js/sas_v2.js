 ...
 * 
 * 修改说明：
 * 1. 彻底移除了导致 TypeError 的 updateNavigation 函数。
 * 2. 增加了全局启动日志以确认版本。
 * 3. 对所有 DOM 元素增加了严格的 null 检查。
 */

console.log("=== SAS V2 SCRIPT INITIALIZING ===");

(function() {
    // 启动初始化
    function init() {
        console.log("=== SAS V2 SCRIPT STARTING ===");
        
        const testForm = document.getElementById("psychologyTest");
        if (!testForm) {
            console.warn("SAS V2: form 'psychologyTest' not found. This script may be running on the wrong page.");
            return;
        }
        ...
        console.log("SAS V2: Elements found, registering onsubmit handler.");

        // 绑定提交事件
        testForm.onsubmit = function(event) {
            event.preventDefault();
            console.log("SAS V2: Form submitted, calculating scores...");
            ...
            if (!isAllAnswered) {
                ...
                console.log("SAS V2: Validation failed - " + msg);
            ...
            if (!resultDisplay) {
                console.error("SAS V2: 'resultDisplay' element not found.");
                return;
            }
    ...
    function addExportButton(container) {
        ...
        btn.onclick = function() {
            ...
            const a = document.createElement('a'); a.href = url; a.download = 'SAS焦虑测评结果.html'; a.click();
        };
    ...
    function addEmailSection(container, score, level) {
        ...
        document.getElementById('send-report-btn').onclick = function() {
            ...
            fetch('https://script.google.com/macros/s/AKfycbwWe9Pld6ZXPIZhSxgtLYpBJ7Qlc-1ljD7pwOMe7dL-Cw4NwV_W6q0XZP7paupeCWoK3g/exec', {
                method: 'POST',
                body: JSON.stringify({ email: email, subject: 'SAS 焦虑自评量表测评报告', body: `<div style="font-family:sans-serif;padding:20px;">${cardContent}${imgData ? `<br><img src="${imgData}" style="max-width:100%;">` : ''}</div>` })
